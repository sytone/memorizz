...
class CosmosDBTools:
    def __init__(self, config: CosmosDBToolsConfig = CosmosDBToolsConfig()):
        self.config = config
        ...
        if self.config.connection_string is None:
            self.config.connection_string = os.getenv('COSMOSDB_CONNECTION_STRING') or getpass.getpass("Enter CosmosDB connection string: ")
        ...
        try:
            self.cosmos_client = pymongo.MongoClient(self.config.connection_string, appname="memorizz.python.package")
            self.db = self.cosmos_client[self.config.db_name]
            ...
            logger.info("CosmosDBTools initialized successfully.")

        except pymongo.errors.ConnectionFailure:
            logger.error("Failed to connect to CosmosDB. Please check your connection string and network.")
        except pymongo.errors.OperationFailure as e:
            if e.code == 13:  # Authentication failed
                logger.error("CosmosDB authentication failed. Please check your credentials.")
            else:
                logger.error(f"CosmosDB operation failed: {str(e)}")
        except Exception as e:
            logger.error(f"Unexpected error during CosmosDB initialization: {str(e)}")
        
        if self.tools_collection is None:
            logger.warning("CosmosDBTools initialization failed. Some features may not work.")

    def cosmosdb_toolbox(self, collection: Optional[Collection] = None):
    ...
    def create_toolbox(self, collection_name: str, vector_index_definition: Dict[str, Any], index_name: str = "vector_index"):
        """
        Create a collection in the CosmosDB database and set up a vector search index.
...
__all__ = ['CosmosDBTools', 'CosmosDBToolsConfig']

# You can create a function to get the cosmosdb_toolbox decorator:
def get_cosmosdb_toolbox(config: CosmosDBToolsConfig = CosmosDBToolsConfig()):
    return CosmosDBTools(config).cosmosdb_toolbox