...
class CosmosDBConfig():
    ...
    def __init__(self, connection_string: str, db_name: str = "memorizz"):
        """
        Initialize the CosmosDB provider with configuration settings.
...
class CosmosDBProvider(MemoryProvider):
    """CosmosDB implementation of the MemoryProvider interface."""
    ...
    def __init__(self, config: CosmosDBConfig):
        ...
        # Create all memory stores in CosmosDB.
        self._create_memory_stores()

        # Create a vector index for each memory store in CosmosDB.
    ...
    def _create_memory_store(self, memory_store_type: MemoryType) -> None:
        """
        Create a new memory store in CosmosDB.
    ...
    def store(self, data: Dict[str, Any], memory_store_type: MemoryType) -> str:
        """
        Store data in CosmosDB using only _id field as primary key.
        ...
        --------
        str
            The ID of the inserted/updated document (CosmosDB _id).
        """
        ...
        # Clean data by removing custom ID fields - only use CosmosDB _id
        ...
        # If document has CosmosDB _id, update it
        if "_id" in data_copy:
            ...
            # For new documents, let CosmosDB generate _id automatically
    ...
    def retrieve_by_query(self, query: Dict[str, Any], memory_store_type: MemoryType, limit: int = 1) -> Optional[Dict[str, Any]]:
        """
        Retrieve a document from CosmosDB.
    ...
    def retrieve_by_id(self, id: str, memory_store_type: MemoryType) -> Optional[Dict[str, Any]]:
        """
        Retrieve a document from CosmosDB by _id.

        Parameters:
        -----------
        id : str
            The CosmosDB _id of the document to retrieve.
        memory_store_type : MemoryType
            The type of memory store (e.g., "persona", "toolbox", etc.)
        ...
        # Retrieve using CosmosDB _id only
        try:
    ...
    def retrieve_by_name(self, name: str, memory_store_type: MemoryType) -> Optional[Dict[str, Any]]:
        """
        Retrieve a document from CosmosDB by name.
    ...
    def retrieve_persona_by_query(self, query: Dict[str, Any], limit: int = 1) -> Optional[Dict[str, Any]]:
        """
        Retrieve a persona or several personas from CosmosDB.
    ...
    def retrieve_toolbox_item(self, query: Dict[str, Any], limit: int = 1) -> Optional[Dict[str, Any]]:
        """
        Retrieve a toolbox item or several items from CosmosDB.
    ...
    def retrieve_workflow_by_query(self, query: Dict[str, Any], limit: int = 1) -> Optional[Dict[str, Any]]:
        """
        Retrieve a workflow or several workflows from CosmosDB.
    ...
    def delete_by_id(self, id: str, memory_store_type: MemoryType) -> bool:
        """
        Delete a document from CosmosDB by _id.
        
        Parameters:
        -----------
        id : str
            The CosmosDB _id of the document to delete.
        memory_store_type : MemoryType
            The type of memory store (e.g., "persona", "toolbox", etc.)
        ...
        # Delete using CosmosDB _id only
        try:
            if ObjectId.is_valid(id):
    ...
    def delete_by_name(self, name: str, memory_store_type: MemoryType) -> bool:
        """
        Delete a document from CosmosDB by name.
    ...
    def delete_all(self, memory_store_type: MemoryType) -> bool:
        """
        Delete all documents within a memory store type in CosmosDB.
    ...
    def list_all(self, memory_store_type: MemoryType) -> List[Dict[str, Any]]:
        """
        List all documents within a memory store type in CosmosDB.
        ...
        --------
        List[Dict[str, Any]]
            The list of all documents from CosmosDB.
        """
    ...
    def update_by_id(self, id: str, data: Dict[str, Any], memory_store_type: MemoryType) -> bool:
        """
        Update a document in a memory store type in CosmosDB by _id.

        Parameters:
        -----------
        id : str
            The CosmosDB _id of the document to update.
        data : Dict[str, Any]
        ...
        # Update using CosmosDB _id only
        try:
            if ObjectId.is_valid(id):
    ...
    def update_toolbox_item(self, id: str, data: Dict[str, Any]) -> bool:
        """
        Update a toolbox item in CosmosDB by id using optimized queries.
        """
    ...
    def store_memagent(self, memagent: "MemAgentModel") -> "MemAgentModel":
        """
        Store a memagent in the CosmosDB database using only _id field.
        ...
        # Insert the document and let CosmosDB generate _id automatically
    ...
    def update_memagent(self, memagent: "MemAgentModel") -> "MemAgentModel":
        """
        Update a memagent in the CosmosDB database using _id field.
        """
        # Convert the MemAgentModel to a dictionary
        memagent_dict = memagent.model_dump()
        ...
        # Update the memagent in the CosmosDB database using _id
    ...
    def retrieve_memagent(self, agent_id: str) -> "MemAgentModel":
        """
        Retrieve a memagent from the CosmosDB database using _id field.
        
        Parameters:
        -----------
        agent_id : str
            The agent ID to retrieve (CosmosDB _id).
        ...
        """
        # Get the document from CosmosDB using _id
        try:
        ...
        # Use the CosmosDB _id as agent_id since we no longer store agent_id field
        memagent = MemAgentModel(
    ...
    def list_memagents(self) -> List["MemAgentModel"]:
        """
        List all memagents in the CosmosDB database.
    ...
    def update_memagent_memory_ids(self, agent_id: str, memory_ids: List[str]) -> bool:
        ...
        -----------
        agent_id : str
            The id of the memagent to update (CosmosDB _id).
        memory_ids : List[str]
    ...
    def _setup_vector_search_index(self, collection, index_name="vector_index", memory_store: bool = False):
        """
        Setup a vector search index for a CosmosDB collection and wait for it to become queryable.

        Args:
        collection: CosmosDB collection object
    ...
    def _wait_for_index_ready(self, collection, index_name_result, display_name="vector_index"):
        """
        Wait for a CosmosDB search index to become queryable using polling.
    ...
    def close(self) -> None:
        """Close the connection to CosmosDB."""
        self.client.close()